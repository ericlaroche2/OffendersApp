...
public class BarcodeSample1 extends Activity implements EMDKListener, DataListener, StatusListener, ScannerConnectionListener, OnCheckedChangeListener {
    ...
    ArrayList<Offender>scannedOffenders=new ArrayList<Offender>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mainbarcodesample);
...
//remove
    ...
    @Override //when a barcode is scanned
    public void onData(ScanDataCollection scanDataCollection) {
        ...
        if ((scanDataCollection != null) && (scanDataCollection.getResult() == ScannerResults.SUCCESS)) {
            ...
            for(ScanData data : scanData) {
                ...
                if (checkBoxContinuous.isChecked()==false){
                    for (int i =0;i<Offenders.size(); i++){
                   //     String test= Offenders.get(i).getID();
                        if (dataString.equals(Offenders.get(i).getID())){
                            Intent intent = new Intent( this,OffenderScreen.class);
                            String openName=Offenders.get(i).getLastName()+", "+ Offenders.get(i).getName()+ "  " + Offenders.get(i).getID() ;
                            intent.putExtra(NAME_SEARCH, openName);
                            intent.putExtra(ID, Offenders.get(i).getID());
                            intent.putExtra(addLocal, Offenders.get(i).getLocation());
                    ...
                    for (int i =0;i<Offenders.size(); i++){
                        ...
                        scannedOffenders.add(Offenders.get(i));
                        }
                    }
                }
                //barcode data here
    ...
    @Override //status of the scanner
    ...
    private void enumerateScannerDevices() {
        ...
        if (barcodeManager != null) {
            ...
            ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(BarcodeSample1.this, android.R.layout.simple_spinner_item, friendlyNameList);
    ...
    private void populateTriggers() {
        ...
        ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(BarcodeSample1.this, android.R.layout.simple_spinner_item, triggerStrings);
        spinnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    ...
    private void initScanner() {
        ...
        if (scanner == null) {
          //from enumeratescanner devices  deviceList = barcodeManager.getSupportedDevicesInfo();
            if ((deviceList != null) && (deviceList.size() != 0)) {
    ...
    private class AsyncDataUpdate extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            if (result != null) {
                ...
                tl = (TableLayout) findViewById(R.id.tableScan);
                TableRow tr = new TableRow(BarcodeSample1.this);
                tr.setLayoutParams(new TableRow.LayoutParams(
                        TableRow.LayoutParams.MATCH_PARENT,
                        TableRow.LayoutParams.WRAP_CONTENT));

                TextView row  = new TextView(BarcodeSample1.this);
                for (int i =0;i<Offenders.size(); i++){
                    //     String test= Offenders.get(i).getID();
                    if (result.equals(Offenders.get(i).getID())){
                        String namescan=result+" "+ Offenders.get(i).getLastName()+", "+Offenders.get(i).getName();
                        row.setText(namescan);
                    }
                }
    ...
    public void modifyGroup(View view){
        ...
        if (scannedOffenders.isEmpty()){
            //display message if no offenders have been scanned
            Snackbar noScannedOffender= Snackbar.make(view, getString(R.string.noId), duration);
            noScannedOffender.show();
        }else{
        Intent intent = new Intent(this, Multichange.class);
        intent.putExtra("offendersList", scannedOffenders);
        ...
        for (int i = 0; i < scannedOffenders.size() ; i++ ){
                name.add(scannedOffenders.get(i).getLastName()+", "+ scannedOffenders.get(i).getName()+ "  " + scannedOffenders.get(i).getID());
                sendID.add(scannedOffenders.get(i).getID());
                position.add(i);
        }
    ...
    public void removeScannedOffender(View view){

        tl.removeViewAt(tl.getChildCount()-1);
        scannedOffenders.remove(scannedOffenders.size()-1);
    }
}
